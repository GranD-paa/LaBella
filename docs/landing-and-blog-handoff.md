...
**دیتابیس**
- `db/004_landing_and_blog.sql` — **اجرا شد (۱۴۰۵/۰۶/۲۰)** روی کلاستر زنده؛ هر چهار جدول ساخته شد و پنج دستهٔ اولیه seed شد
...
### ~~الف) مایگریشن دیتابیس~~ — انجام شد ۱۴۰۵/۰۶/۲۰

```bash
psql "$DATABASE_URL" -f "db/004_landing_and_blog.sql"
```

نبودنش تا آن روز پیدا نشده بود چون `/blog` خطا را می‌گرفت و بی‌صدا حالت خالی
نشان می‌داد. چیزی که بالاخره لوش داد، `/admin/blog` بود که محافظ نداشت و ۵۰۰
می‌داد. حالا هر دو مسیر ادمین هم محافظ دارند و اسم همین مایگریشن را می‌برند.
