...
drop function if exists public.create_pending_payment(text, text, text, text, text);
...
-- to expose: it is scoped to the caller's own id and settles nothing by itself.
create or replace function public.list_my_pending_payments(p_user_id text)
returns setof public.payments
language sql
security definer
set search_path = ''
